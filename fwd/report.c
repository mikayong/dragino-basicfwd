...
static void semtech_report(serv_s *serv) {
    ...
    lgw_log(LOG_REPORT, "# RF packets forwarded: %u (%u bytes)\n", cp_up_pkt_fwd, cp_up_payload_byte);
    lgw_log(LOG_REPORT, "# PUSH_DATA datagrams sent: %u (%u bytes)\n", cp_up_dgram_sent, cp_up_network_byte);
    lgw_log(LOG_REPORT, "# PUSH_DATA acknowledged: %.2f%%\n", 100.0 * up_ack_ratio);

    lgw_log(LOG_REPORT, "### [DOWNSTREAM] ###\n");
    lgw_log(LOG_REPORT, "# PULL_DATA sent: %u (%.2f%% acknowledged)\n", cp_dw_pull_sent, 100.0 * dw_ack_ratio);
    lgw_log(LOG_REPORT, "# PULL_RESP(onse) datagrams received: %u (%u bytes)\n", cp_dw_dgram_rcv, cp_dw_network_byte);
    lgw_log(LOG_REPORT, "# RF packets sent to concentrator: %u (%u bytes)\n", (cp_nb_tx_ok + cp_nb_tx_fail), cp_dw_payload_byte);
    lgw_log(LOG_REPORT, "# TX errors: %u\n", cp_nb_tx_fail);

    if (cp_nb_tx_requested != 0) {
        lgw_log(LOG_REPORT, "# TX rejected (collision packet): %.2f%% (req:%u, rej:%u)\n",
                   100.0 * cp_nb_tx_rejected_collision_packet / cp_nb_tx_requested, 
                   cp_nb_tx_requested,
                   cp_nb_tx_rejected_collision_packet);
        lgw_log(LOG_REPORT, "# TX rejected (collision beacon): %.2f%% (req:%u, rej:%u)\n",
                   100.0 * cp_nb_tx_rejected_collision_beacon / cp_nb_tx_requested, 
                   cp_nb_tx_requested,
                   cp_nb_tx_rejected_collision_beacon);
        lgw_log(LOG_REPORT, "# TX rejected (too late): %.2f%% (req:%u, rej:%u)\n",
                   100.0 * cp_nb_tx_rejected_too_late / cp_nb_tx_requested,
                   cp_nb_tx_requested, 
                   cp_nb_tx_rejected_too_late);
        lgw_log(LOG_REPORT, "# TX rejected (too early): %.2f%% (req:%u, rej:%u)\n",
                   100.0 * cp_nb_tx_rejected_too_early / cp_nb_tx_requested,
                   cp_nb_tx_requested, 
                   cp_nb_tx_rejected_too_early);
    }
    if (!strncmp(GW.hal.board, "sx1302", 6)) {
        lgw_log(LOG_REPORT, "\n### [SX1302 status] ###\n");
        pthread_mutex_lock(&GW.hal.mx_concent);
        i = lgw_get_trigcnt(&trigcnt);
        i |= lgw_get_instcnt(&instcnt);
    ...
    if (i != LGW_HAL_SUCCESS) {
        lgw_log(LOG_REPORT, "# SX130X counter unknown\n");
    } else {
        lgw_log(LOG_REPORT, "# SX130X counter (INST): %u\n", instcnt);
        lgw_log(LOG_REPORT, "# SX130X counter (PPS): %u\n", trigcnt);
        lgw_log(LOG_REPORT, "# Concentrator temperature: %.0f C #\n", temperature);
    }

    lgw_log(LOG_REPORT, "\n### [BEACON] ###\n");
    lgw_log(LOG_REPORT, "# Packets queued: %u\n", cp_nb_beacon_queued);
    lgw_log(LOG_REPORT, "# Packets sent so far: %u\n", cp_nb_beacon_sent);
    lgw_log(LOG_REPORT, "# Packets rejected: %u\n", cp_nb_beacon_rejected);

    lgw_log(LOG_REPORT, "### [JIT] ###\n");
    ...
    if (GW.gps.gps_enabled == true) {
        ...
        if (GW.gps.gps_ref_valid == true) {
            ...
            lgw_log(LOG_REPORT, "# Invalid gps time reference (age: %li sec)\n", (long)difftime(time(NULL), GW.gps.time_reference_gps.systime));
        }

        if (coord_ok) {
            lgw_log(LOG_REPORT, "# System GPS coordinates: latitude %.5f, longitude %.5f, altitude %i m\n",
                    cp_gps_coord.lat, cp_gps_coord.lon, cp_gps_coord.alt);
        } else {
            lgw_log(LOG_REPORT, "# no valid GPS coordinates available yet\n");
        }
    } else if (GW.gps.gps_fake_enable == true) {
        lgw_log(LOG_REPORT, "# GPS *FAKE* coordinates: latitude %.5f, longitude %.5f, altitude %i m\n", cp_gps_coord.lat, cp_gps_coord.lon, cp_gps_coord.alt);
    } else {
