...
int sx126x_radio_configure_tx( const ral_t* ral, struct lgw_pkt_tx_s* pkt_data )
{
    set_led_rx( ral, false );
    set_led_tx( ral, true );

    /* Check parameters */
    if( sx126x_check_lora_mod_params( pkt_data->freq_hz, pkt_data->bandwidth, pkt_data->coderate ) != LGW_HAL_SUCCESS )
    {
        printf( "Invalid parameters to configure for TX" );
    ...
    ral_lora_sf_t         ral_sf          = sx126x_convert_hal_to_ral_sf( pkt_data->datarate );
    ral_lora_bw_t         ral_bw          = sx126x_convert_hal_to_ral_bw( pkt_data->bandwidth );
    ral_lora_cr_t         ral_cr          = sx126x_convert_hal_to_ral_cr( pkt_data->coderate );
    ...
    ASSERT_RAL_RC( ral_set_lora_sync_word( ral, sx126x_get_lora_sync_word( pkt_data->freq_hz, pkt_data->datarate ) ) );
