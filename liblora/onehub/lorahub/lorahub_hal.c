...
int sx126x_receive( uint8_t max_pkt, struct sx126x_pkt_rx_s* pkt_data )
{
    struct sx126x_pkt_rx_s* p = &pkt_data[0];
    uint32_t             count_us;
    int8_t               rssi, snr;
    uint8_t              status, sf;
    uint16_t             size;
    ...
    memset( p, 0, sizeof( struct sx126x_pkt_rx_s ) );
    nb_packet_received =
...
int sx126x_send( struct lgw_pkt_tx_s* pkt_data )
{
    ...
    printf( "TCXO startup time: %lu", tcxo_startup_time_us );
